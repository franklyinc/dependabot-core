...
RSpec.describe Dependabot::Terraform::FileParser do
  ...
  let(:terraform_fixture_name) { "git_tags_011.tf" }
  ...
  describe "parse" do
    ...
    context "with registry sources for 0.11" do
      let(:terraform_fixture_name) { "registry_011.tf" }
    ...
    context "with registry sources for 0.12" do
      let(:terraform_fixture_name) { "registry_012.tf" }
    ...
    context "with git sources for 0.11" do
      let(:terraform_fixture_name) { "git_tags_011.tf" }
    ...
    context "with git sources for 0.12" do
      let(:terraform_fixture_name) { "git_tags_012.tf" }
        ...
        let(:expected_requirements) do
          [{
            ...
            source: {
              ...
              "terraform-aws-route53-cluster-zone.git",
              branch: nil,
              ref: "0.2.5"
            }
          }]
      ...
      describe "the third dependency (which uses github.com with a tag)" do
        subject(:dependency) { dependencies[2] }
        let(:expected_requirements) do
          [{
            requirement: nil,
        ...
        it "has the right details" do
          expect(dependency).to be_a(Dependabot::Dependency)
          expect(dependency.name).to eq("duplicate_label")
          expect(dependency.version).to eq("0.3.7")
          expect(dependency.requirements).to eq(expected_requirements)
        end
      end
    ...
    context "with a terragrunt file" do
      ...
      let(:terragrunt_fixture_name) { "terragrunt.hcl" }
